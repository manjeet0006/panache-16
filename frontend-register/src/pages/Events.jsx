              ...
              <div className="p-4 bg-white/[0.03] rounded-3xl border border-white/5 backdrop-blur-sm flex flex-col gap-4">
                <div className="flex flex-wrap justify-center gap-2">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${selectedCategory === "ALL" ? "bg-white text-black border-white" : "bg-transparent text-gray-500 border-transparent hover:bg-white/5"}`}
                  >
                    All
                  </button>
                  ...
                  {CATEGORIES.map((cat) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border transition-all text-[10px] font-black uppercase tracking-widest ${isActive ? "bg-pink-500 border-pink-500 text-white shadow-lg shadow-pink-500/20" : "bg-white/5 border-white/5 text-gray-400 hover:border-pink-500/30 hover:text-white"}`}
                      >
                    ...
                    {["FREE", "PAID"].map((type) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-all ${isActive ? "bg-white text-black border-white" : "bg-transparent text-gray-500 border-white/10 hover:border-white/30"}`}
                        >
                          {isActive && <X size={12} />}
                          {type} Only
                        </button>
        ...
        <div className="grid grid-cols-1 p-3 md:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
          ...
          {filteredEvents.length === 0 ? (
            <div className="col-span-full py-20 text-center rounded-[3rem] border border-dashed border-white/10">
              ...
              <button onClick={() => { setSearchQuery(""); setSelectedCategory("ALL"); setPriceFilter("ALL"); }} className="mt-4 text-pink-500 text-[12px] font-bold uppercase tracking-widest underline">
                Reset Filters
              </button>
            </div>
          ) : (
            filteredEvents.map(event => (
              <div
                key={event.id}
                // Removed h-full so it doesn't force height
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  {/* Title: Fixed height + line clamp ensures uniform start */}
                  <div className="h-[4.5rem] mb-2 flex items-center">
                ...
                <div className="relative z-10 pt-3 border-t border-white/5 flex flex-col gap-6">
                  ...
                  <button
                    ...
                    className="w-full bg-white text-black py-4 rounded-xl font-extrabold uppercase text-[14px] tracking-[0.2em] transition-all hover:bg-pink-500 hover:text-white shadow-xl active:scale-[0.98] group-hover:shadow-pink-500/10"
                  >
                    Start Registration
                  </button>
                </div>
