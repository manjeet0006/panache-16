...
import {
    ...
    Sparkles, User, Mail, GraduationCap, X
} from 'lucide-react';
import API from '../api';
...
const Dashboard = () => {
        ...
        return (
             <div className="min-h-screen bg-[#050505] text-white p-6 md:p-12 animate-in slide-in-from-bottom-10 duration-500 z-[100] relative">
                ...
                <div className="max-w-5xl mx-auto relative z-10">
                    ...
                    {/* Header / Nav */}
                    <div className="flex pt-13 items-center justify-between mb-8">
                        <button 
                        ...
                        <div className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                                    ...
                                    {inspectingTeam.members?.map((member, idx) => (
                                        <div key={idx} className="group p-4 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-pink-500/30 transition-all">
                                            <div className="flex items-start gap-4">
                                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-800 to-black flex items-center justify-center border border-white/10 text-xs font-bold">
                                                    {idx + 1}
                                                </div>
                                                <div className="overflow-hidden">
                                                    ...
                                                    {member.panacheId && (
                                                        <div className="flex items-center gap-2 text-[10px] text-gray-600 mt-1 uppercase tracking-wider font-mono">
                                                            <Hash size={10} />
                                                            <span>{member.panacheId}</span>
                                                        </div>
                                                    )}
                                                    {idx === 0 && (
                                                        <span className="inline-block mt-2 text-[9px] font-bold text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded uppercase tracking-widest">
                                                            Team Lead
                                                        </span>
                                                    )}
                                                </div>
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-6">
                                ...
                                <div className="p-4 rounded-xl bg-black/40 border border-white/5 space-y-3">
                                    ...
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <span className="font-mono text-xs">{inspectingTeam.transactionId || "N/A"}</span>
                                    </div>
                                    <div className="h-px bg-white/10 my-2"></div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-gray-500 text-sm">Total Paid</span>
                                        <span className="text-xl font-bold text-green-400">₹{inspectingTeam.event?.eventPrice || 0}</span>
                                    </div>
                                </div>
                            </div>
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white flex flex-col md:flex-row relative overflow-hidden selection:bg-pink-500/30">
            ...
            <aside className="w-full md:w-72 md:h-screen md:sticky md:top-0 bg-[#0a0a0a]/90 backdrop-blur-xl border-b md:border-b-0 md:border-r border-white/5 p-6 md:p-8 flex flex-row md:flex-col justify-between z-50 shrink-0">
                ...
                <div className="hidden md:flex flex-col gap-6 mt-12">
                    ...
                    <nav className="space-y-2">
                        <div className="w-full flex items-center gap-3 px-4 py-3 bg-pink-500 text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-pink-500/20 cursor-default">
                            <Ticket size={16} /> My Tickets
                        </div>
                        <button onClick={() => window.location.href='/events'} className="w-full flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all text-left">
            ...
            {/* 2. MAIN CONTENT AREA */}
            <main className="flex-1 mt-17 p-4 md:p-10 md:h-screen md:overflow-y-auto z-10 scroll-smooth">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 mb-8 md:mb-10">
                    <div className="bg-white/[0.02] border border-white/5 p-4 md:p-6 rounded-[2rem]">
                        <div className="flex justify-between mb-4">
                            <div className="p-2 md:p-3 bg-white/5 rounded-2xl text-white"><LayoutGrid size={18} /></div>
                        </div>
                        <p className="text-[9px] md:text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Registrations</p>
                        <p className="text-2xl md:text-3xl font-black italic">{teams.length}</p>
                    </div>
                    <div className="bg-white/[0.02] border border-white/5 p-4 md:p-6 rounded-[2rem]">
                        <div className="flex justify-between mb-4">
                            <div className="p-2 md:p-3 bg-green-500/10 text-green-500 rounded-2xl"><CheckCircle size={18} /></div>
                        </div>
                        <p className="text-[9px] md:text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Verified</p>
                    ...
                    <div className="bg-white/[0.02] border border-white/5 p-4 md:p-6 rounded-[2rem] col-span-2 md:col-span-1">
                        <div className="flex justify-between mb-4">
                            <div className="p-2 md:p-3 bg-purple-500/10 text-purple-500 rounded-2xl"><Users size={18} /></div>
                        </div>
                        <p className="text-[9px] md:text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Teammates</p>
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
                        {['ALL', 'PANACHE', 'PRATISHTHA', 'PRAGATI'].map((cat) => (
                            <button
                                ...
                                className={`px-4 py-3 md:px-5 md:py-4 rounded-2xl text-[9px] font-bold uppercase tracking-widest transition-all whitespace-nowrap ${activeFilter === cat ? 'bg-pink-500 text-white shadow-lg shadow-pink-500/20' : 'bg-white/5 text-gray-500 border border-white/5 hover:bg-white/10'}`}
                            >
                                {cat}
                            </button>
                        ))}
                                ...
                                >
                                    <div className="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
                                        <div className="text-center md:text-left">
                                            ...
                                            <h3 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter group-hover:text-pink-500 transition-colors text-white">
                                                {team.teamName}
                                            </h3>
