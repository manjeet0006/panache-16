...
const Header = () => {
  ...
  const navLinks = [
    ...
    { name: 'Concert Ticket', path: '/login', icon: Ticket },
          ...
          <div className="hidden md:flex items-center gap-1">
                  ...
                  <div className="absolute top-full right-0 mt-4 w-48 bg-[#0A0A0A] border border-white/10 rounded-2xl p-2 shadow-2xl opacity-0 translate-y-2 invisible group-hover:opacity-100 group-hover:translate-y-0 group-hover:visible transition-all duration-300 transform origin-top-right">
                    {dropdownLinks.map((link) => (
                      <Link
                        key={link.name}
                        to={link.path}
                        className={`flex items-center gap-3 w-full p-3 rounded-xl transition-all ${location.pathname === link.path ? 'bg-pink-500 text-white' : 'hover:bg-white/5 text-gray-400 hover:text-white'
                          }`}
                      >
                        <link.icon size={16} />
            ...
            <div className="pl-6 border-l border-white/10">
              {token ? (
                ...
                <Link
                  to="/login"
                  className="group flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 hover:border-white/30 hover:bg-white/5 transition-all duration-300"
                >
                  <LogIn size={16} className="text-gray-500 group-hover:text-white transition-colors" />
                  <span className="text-[12px] font-bold uppercase tracking-widest text-gray-400 group-hover:text-white transition-colors">
                    Login
                  </span>
                </Link>
              )}
